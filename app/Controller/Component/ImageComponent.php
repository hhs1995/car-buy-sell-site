...
class ImageComponent extends Component
{
    ...
    */
    ...
    function upload_image_and_thumbnail($data, $datakey, $folderName, $name_id = null ,$tamanios = null)
    {
		if (strlen($data['name']) > 4) {
            
			/*PISO LOS TAMANIOS CON EL ARRAY QUE LE MANDO*/
			if(isset($tamanios) && !empty($tamanios)){
            ...
            // Generate a unique name for the image
           	...
           	$filename .= (isset($name_id) && !empty($name_id) ? '-'.$name_id : '').".";
           	$filename .= $filetype;
            ...
            if (is_uploaded_file($data['tmp_name'])) {
                ...
                if (!copy($data['tmp_name'], "$tempfile")) {
                	...
                	foreach( $this->tamanios as $tam => $tam_conf ) {
						...
						/*PARTE AGREGADA POR MI PARA FORZAR TAMANIOS*/                		                		
                		/*if(isset($tam) && ($tam == 'slider' || $tam == 'destacado1' || $tam == 'destacado2')){
                			$tam_conf['width'] = $tamanios_forzados[$tam]['width'];
                			$tam_conf['height'] = $tamanios_forzados[$tam]['height'];
                		}*/            		
