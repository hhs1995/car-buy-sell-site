...
(function($) {
	...
	$.fn.coinslider= $.fn.CoinSlider = function(options){
		...
		$.setNavigation = function(el){
			...
			for(k=1;k<images[el.id].length+1;k++){
				$('#cs-buttons-'+el.id).append("<a href='#' class='cs-button-"+el.id+"' id='cs-button-"+el.id+"-"+k+"'><i></i><span></span></a>");
				$('#cs-button-'+el.id+'-'+k+' span').html(titles[el.id][(k-1)]);	
				var imgname = $('#cs-button-'+el.id+'-'+k+' span p.logo').html();
				$('#cs-button-'+el.id+'-'+k+' span p.logo').html('');	
				$('#cs-button-'+el.id+'-'+k+' span p.logo').css("background-image", "url('http://tuplanya.com/img/tuplanya/"+imgname+"')");	
			}
	...
	$.fn.coinslider.defaults = {	
		width: 1005, // width of slider panel
		height: 253, // height of slider panel
