...
Admin.tooltips = function(){
	$.each(tooltips, function (key, el){
		$(el.jq_element).append(' <a class="tooltip" title="'+el.text+'"><span class="ui-icon ui-icon-info"></span></a>');
	})
}
...
$(document).ready(function() {
	...
	$('.tabs').tabs();
    $('a.tooltip').tipsy({ offset:100, opacity:0.1,gravity: 'w', delayIn:2000, fade:true});
    $('a.tooltip_s').tipsy({ offset:100, opacity:0.1,gravity: 's', delayIn:2000, fade:true});
    $('textarea.elastico').not('.file-content, #NodeBody').elastic();
});